...
public class VideoProcessor {
    ...
    private OnVideoProcessListener listener;

    private int mBitRate = 3000000;
    private boolean mNeedAudio = false;
    private Context mContext;
    private boolean mVideoEncodeInitSucceed = false;
    ...
    private boolean mNeedDetectAnimal = false;
    public void enableAnimalDetect(){
        mNeedDetectAnimal = true;
    }

    private String bodyModelName = "";

    public void setBodyModelName(String bodyModelName) {
        this.bodyModelName = bodyModelName;
    }

    public void setOnVideoCutFinishListener(OnVideoProcessListener listener) {
        this.listener = listener;
    }

    public void setOnFrameCallbackListener(FrameCallbackListener listener) {
        this.mFrameCallbackListener = listener;
    }
     ...
     * @throws IOException
     */
    public void processVideo(long startPosition, long clipDur) throws IOException {
...
//                muxAudioTrack = mMediaMuxer.addTrack(format);
    ...
    private Runnable videoCliper = new Runnable() {
        ...
        public void run() {
            ...
            mVideoEncodeInitSucceed = initVideoCodec();//暂时统一处理,为音频转换采样率做准备

            if(mVideoEncodeInitSucceed){
    ...
    private Runnable audioCliper = new Runnable() {
        ...
        public void run() {
            ...
            if(audioTrackIndex >= 0 && Build.VERSION.SDK_INT >= 21){
                mNeedAudio = true;
            }
    ...
    private boolean initAudioCodec() {
        try{
            ...
            Log.e("sensetime", "initAudioCodec: "+e.getMessage() );
        ...
        while (!done  && !mNeedStop) {
            if (!inputDone) {
                ...
                if (inputIndex >= 0) {
                    ...
                    if ((dur < duration) && readSampleData > 0 && !mNeedStop) {
                        decoder.queueInputBuffer(inputIndex, 0, readSampleData, extractor.getSampleTime(), 0);
                        decodeinput++;
                        System.out.println("videoCliper audio decodeinput" + decodeinput + " dataSize" + readSampleData + " sampleTime" + extractor.getSampleTime());
                        extractor.advance();
                    } else {
                ...
                if (index == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    ...
                    if (canEncode && !endOfStream) {
                        ...
                        if (encodeInputIndex >= 0) {
                            ...
                            if (info.size < 4096) {//这里看起来应该是16位单声道转16位双声道
                                ...
                                encodeinput++;
                                System.out.println("videoCliper audio encodeInput" + encodeinput + " dataSize" + info.size + " sampleTime" + info.presentationTimeUs);
                            } else {
                    ...
                    if (endOfStream) {
                        try{
                            ...
                            Log.e("videoCliper", "audio end error");
                            mNeedStop = true;
                        }
            ...
            while (encoderOutputAvailable  && !mNeedStop) {
    ...
    private boolean initVideoCodec() {
        //不对视频进行压缩
        ...
        if(info.height > 3500 || info.width > 3500){
            mBitRate = 10000000;
        }
        ...
        if(mDetectConfig != 0){
            outputSurface.setDetectConfig(mDetectConfig);
        }

        if(mNeedDetectAnimal){
            outputSurface.enableAnimalDetect();
        }

        outputSurface.setRenderOptions(mNeedBeautify, mNeedFilter, mNeedSticker, mNeedMakeup);
        outputSurface.setCurrentStickerPath(mCurrentStickerPath);
        outputSurface.setCurrentBeautyParams(mBaseBeautyParams, mProfessionalBeautyParams, mMicroBeautyParams, mAdjustBeautyParams);
        outputSurface.setCurrentMakeups(mCurrentMakeupPaths, mCurrentMakeupStrengths);
        outputSurface.setCurrentFilter(mCurrentFilterPath, mCurrentFilterStrength);
        ...
        while (!done && !mNeedStop) {
            if (!inputDone) {
                ...
                if (inputIndex >= 0) {
                    ...
                    if (mFrameCallbackListener != null) {
                        mFrameCallbackListener.onFrameProgress((float)dur/duration * 100);
                    }
    ...
    private synchronized void forceRelease() {
        if (released) {
            return;
        }
    ...
    public void setCurrentSticker(String path){
        mCurrentStickerPath = path;
    }

    private String[] mCurrentMakeupPaths = new String[Constants.MAKEUP_TYPE_COUNT];
    private float[] mCurrentMakeupStrengths = new float[Constants.MAKEUP_TYPE_COUNT];
    public void setCurrentMakeups(String[] paths, float[] strengths){
        for (int i = 0; i < paths.length; i++){
            mCurrentMakeupPaths[i] = paths[i];
            mCurrentMakeupStrengths[i] = strengths[i];
        }
    }
    ...
    public void setCurrentBeautyParams(float[] paramsBase, float[] paramsProfessional, float[] paramsMicro, float[] paramsAdjust){
        for (int i = 0; i < paramsBase.length; i++){
            mBaseBeautyParams[i] = paramsBase[i];
        }
        for (int i = 0; i < paramsProfessional.length; i++){
            mProfessionalBeautyParams[i] = paramsProfessional[i];
        }
        for (int i = 0; i < paramsMicro.length; i++){
            mMicroBeautyParams[i] = paramsMicro[i];
        }
        for (int i = 0; i < paramsAdjust.length; i++){
            mAdjustBeautyParams[i] = paramsAdjust[i];
        }
    }
    ...
    private FrameCallbackListener mFrameCallbackListener;
