...
public class STHumanAction {
    ...
    STMobileFaceInfo[] faces; ///< 检测到的人脸信息
    int faceCount;            ///< 检测到的人脸数目

    STMobileHandInfo[] hands; ///< 检测到的手的信息
    int handCount;            ///< 检测到的手的数目 (目前仅支持检测一只手)

    STSegment image;            ///< 前后背景分割图片信息,前景为0,背景为255,边缘部分有模糊(0-255之间),输出图像大小可以调节

    STSegment hair;             ///< 头发分割图片信息,前景为0,背景为255,边缘部分有模糊(0-255之间),输出图像大小可以调节
    STSegment skin;             ///< 皮肤分割图片信息,前景为0,背景为255,边缘部分有模糊(0-255之间),输出图像大小可以调节
    STSegment head;             ///< 头部分割图片信息,前景为0,背景为255,边缘部分有模糊(0-255之间),输出图像大小可以调节
    STSegment[] mouthParses;
    int mouthParseCount;

    STSegment multiSegment; ///多类分割结果图片信息,背景0，手1，头发2，眼镜3，躯干4，上臂5，下臂6，大腿7，小腿8，脚9，帽子10，随身物品11，脸12，上衣13，下装14，输出图像大小可以调节

    STMobileBodyInfo[] bodys; ///< 检测到的人体信息
    int bodyCount;            ///< 检测到的人体的数目
    ...
    public void setImage(STSegment image) {
        this.image = image;
    }
    ...
    public void setHair(STSegment hair) {
        this.hair = hair;
    }
    ...
    public void setHead(STSegment head) {
        this.head = head;
    }

    public void setMouthParseCount(int mouthParseCount) {
        this.mouthParseCount = mouthParseCount;
    }

    public void setSkin(STSegment skin) {
        this.skin = skin;
    }

    public void setMouthParses(STSegment[] mouthParses) {
        this.mouthParses = mouthParses;
    }

    public void setMultiSegment(STSegment multiSegment) {
