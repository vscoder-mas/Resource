...
#define ST_MOBILE_SEG_SKY				   0x100000000000000  ///< 检测天空分割
...
#define ST_MOBILE_ENABLE_INPUT_CUSTOM           0x00080000  ///< 使用用户自定义的结果作为输入，必须和视频/图片模式混用，目前暂时无效
...
typedef struct st_mobile_human_action_t {
    ...
    float camera_motion_score;               ///< 摄像头运动状态置信度 暂时不支持
...
#define ST_MOBILE_ENABLE_SKY_SEGMENT			0x10000000000   /// 检测天空分割
...
typedef enum {
	...
	/// 设置背景分割使用的内核种类,默认0是普通分割，2是绿幕分割
	ST_HUMAN_ACTION_PARAM_SEGMENT_KERNAL_TYPE = 25,//已废弃
	/// 设置背景分割边界区域上限阈值.
	...
	ST_HUMAN_ACTION_PARAM_FACE_MESH_MODE = 29,
	/// 设置头发分割边界区域上限阈值.
	...
	// 设置face mesh额头点扩展scale范围起始值（小于终止值，默认是2），只对2106模型有效
	ST_HUMAN_ACTION_PARAM_FACE_MESH_START_SCALE = 38,
	// 设置face mesh额头点扩展scale范围终止值（大于起始值，默认是3），只对2106模型有效
	ST_HUMAN_ACTION_PARAM_FACE_MESH_END_SCALE = 39,
	/// 设置绿幕分割颜色RGB值,默认为绿色,将颜色值（16进制数0xFFFFFF,按R、G、B排列）按float类型传入。
	ST_HUMAN_ACTION_PARAM_GREEN_SEGMENT_COLOR = 40,
	///设置人脸106点检测的阈值[0.0,1.0]
	ST_HUMAN_ACTION_PARAM_FACE_THRESHOLD = 41,
	//设置身体检测的阈值[0.0，1.0]
	ST_HUMAN_ACTION_PARAM_BODY_THRESHOLD = 42,
	//设置手检测的阈值[0.0,1.0]
	ST_HUMAN_ACTION_PARAM_HAND_THRESHOLD = 43,
	//设置手骨架检测的阈值[0.0,1.0]
	ST_HUMAN_ACTION_PARAM_HAND_SKELETON_THRESHOLD = 44,
	// 设置face mesh结果输出坐标系,(0: 屏幕坐标系， 1：三维坐标系，默认是屏幕坐标系）
	ST_HUMAN_ACTION_PARAM_FACE_MESH_OUTPUT_FORMAT = 45,
	//天空分割羽化程度
	ST_HUMAN_ACTION_PARAM_SKY_BLUR_STRENGTH = 46,
	//输出的sky结果中长边的长度
	ST_HUMAN_ACTION_PARAM_SKY_MAX_SIZE = 47,
	//天空分割检测结果灰度图的方向是否需要旋转
	ST_HUMAN_ACTION_PARAM_SKY_RESULT_ROTATE = 48,
	//设置天空分割边界区域上限阈值
	ST_HUMAN_ACTION_PARAM_SKY_SEGMENT_MAX_THRESHOLD = 49,
	//设置天空分割边界区域下限阈值
	ST_HUMAN_ACTION_PARAM_SKY_SEGMENT_MIN_THRESHOLD = 50,
	// 头部结果中长边的长度[10,长边长度](默认长边240,短边=长边/原始图像长边*原始图像短边).值越大,头部分割的耗时越长,边缘部分效果越好.
	ST_HUMAN_ACTION_PARAM_HEAD_MAX_SIZE = 51,
	// 设置face mesh是否需要内缩282关键点坐标(0：不需要内缩，1：需要内缩)
	ST_HUMAN_ACTION_PARAM_FACE_MESH_NARROW_LANDMARK = 52,
...
/// @param[out] p_mouth_eye_mesh 3Dmesh嘴巴和眼睛面片索引结果，底层分配内存
/// @return  成功返回ST_OK, 失败返回其他错误码
ST_SDK_API st_result_t
...
/// @brief 获取3D mesh模型类型信息，2106模型或者3060模型，
/// @param[in] handle 已初始化的human_action句柄
/// @param[out] type 3Dmesh模型信息结果，2106表示2106模型，3060表示3060模型
/// @return  成功返回ST_OK, 失败返回其他错误码
ST_SDK_API st_result_t
st_mobile_human_action_get_face_mesh_model_type(
	st_handle_t handle,
	int *type
);

/// @brief 加载3d mesh的标准人脸obj模型，用于内部的算法处理与mesh输出
/// @param[in] handle 已初始化的human_action句柄
/// @param[in] p_obj_path obj模型的文件路径
/// @return  成功返回ST_OK, 失败返回其他错误码
ST_SDK_API st_result_t
st_mobile_human_action_load_standard_facemesh_obj(
	st_handle_t handle,
...
/// @return  成功返回ST_OK, 失败返回其他错误码
ST_SDK_API st_result_t
st_mobile_human_action_load_standard_facemesh_obj_from_buffer(
	st_handle_t handle,
	const char* p_buffer,
	int buffer_len
);
