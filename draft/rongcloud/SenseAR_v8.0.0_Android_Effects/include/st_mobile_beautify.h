...
typedef enum {
    ...
    ST_BEAUTIFY_WHITEN_MODE = 100,                      /// 美白模式, 0为ST_BEAUTIFY_WHITEN_STRENGTH, 1为ST_BEAUTIFY_WHITEN2_STRENGTH, 2为ST_BEAUTIFY_WHITEN3_STRENGTH

    ST_BEAUTIFY_PARAM_SYNC_OUTPUT = 1000,               /// 同步输出，传入非0值，将禁用内部的双缓冲读取渲染结果的机制，否则在PC上使用双缓冲读取渲染结果（性能高、结果差一帧）
} st_beautify_type;
